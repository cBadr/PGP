...
export function SignForm({ keys }: { keys: Key[] }) {
  ...
  const run = async () => {
    ...
    try {
      if (mode === "server") {
        ...
        await fetch("/api/log", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "sign", details: `client-side · ${key.fingerprint.slice(-16)}` }) });
      }
  ...
  return (
    <div className="glass-card p-6 space-y-5">
      ...
      <div>
        <span className="block text-xs uppercase tracking-widest text-white/50 mb-1.5">Signing key</span>
        {keys.length === 0 ? (
